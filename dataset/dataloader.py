...
def o_list_data_collate(batch: Sequence):
    ...
    for item in tmp_data:
        tmp_item = {}
        item['post_label'] = item['post_label'].float()
        for k in valid_keys:
            tmp_item[k] = item[k]
...
class UniformSmartCacheDataset(CacheDataset):
    def __init__(self, data, transform, cache_rate, datasetkey, num_workers=8):
...
class LoadImageh5d(MapTransform):
    ...
    def __call__(self, data, reader: Optional[ImageReader] = None):
        ...
        with h5py.File(post_label_pth, 'r') as hf:
            data = hf['post_label'][()]
        d['post_label'] = data[0]#.astype(float)
        return d
