...
class ZePT(nn.Module):
    ...
    # def forward(self, x_in, knowledge_embedding):
    #     B = x_in.shape[0]
    ...
    #     know_emb = knowledge_embedding.unsqueeze(0).repeat(B, 1, 1)
    ...
    #     sim_i2t = weight @ know_emb.transpose(1,2) / self.temp
    #     sim_t2i = know_emb @ weight.transpose(1,2) / self.temp   
        
    #     sim_targets = torch.zeros(N, N).to(out.device).float()
    #     sim_targets.fill_diagonal_(1)
    #     sim_targets = sim_targets.unsqueeze(0).repeat(B, 1, 1)
        
    #     loss_i2t = -torch.sum(F.log_softmax(sim_i2t, dim=2)*sim_targets,dim=2).mean()
    #     loss_t2i = -torch.sum(F.log_softmax(sim_t2i, dim=2)*sim_targets,dim=2).mean()
